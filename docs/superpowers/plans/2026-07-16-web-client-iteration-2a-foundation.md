...
> **Правка (373dc24):** код ниже был rules-некорректен — при 3 живых игроках кон закрывается ПО СЧЁТУ на 3 картах (R-5.5), поэтому 4-картовый кон невозможен. Авторитетная версия — в `web/src/fixtures/scenario.ts`: раздача → заход 9♦ → Боря 10♦ → Вера J♦ (**кон закрыт по счёту, R-5.5**) → Вера заходит 7♣ → ваш бой Дамой♥ (**кон закрыт рано, R-3.7.1**, 2<3). Закрывший открывает следующий (R-5.7). Блок ниже оставлен как исходный черновик.
